...
>
> **Tier guidance:** Tier 1 covers the 80% case — the canonical confirmation-modal recipe, the single most common popup bug ("OpenPopup every frame"), context-menu helpers, and common pitfalls. Tier 2 is mechanism — full popup lifecycle, modal vs non-modal explained, closing semantics, ID scoping. Tier 3 is rare cases (nested popups). Default load: Tier 1 only via the Quick navigation below.
...
## 1. Canonical confirmation modal
...
Why each piece is there: `OpenPopup` is gated on the button click — fire-once, NOT every frame (see §2 below). `ImGuiWindowFlags_AlwaysAutoResize` keeps the modal sized to its content; useful when the count or message text varies. The fixed-width buttons (`ImVec2(120, 0)`) keep the layout stable across messages of different length. Both branches call `CloseCurrentPopup`; the imscoped guard's destructor handles `EndPopup` whether the body returned early or ran to completion. To get Escape-to-close, swap the second arg from `nullptr` to a `bool*` you toggle yourself — see Tier 2 §6.

For a non-modal dropdown, swap `ImScoped::PopupModal` for `ImScoped::Popup`. For right-click menus, see §3.

---

## 2. Why "OpenPopup every frame" is wrong
...
For the full lifecycle picture, see Tier 2 §5.

---

## 3. Context-menu helpers
...
For per-row context menus inside a loop, push an `ImScoped::ID` per row to disambiguate (see Tier 2 §8).
...
**"All my per-row context menus open for the first row."**
Loop submitting `BeginPopupContextItem` without an `ImScoped::ID` per iteration. The popup name collides across rows. See Tier 2 §8 for the mechanism.

---

# Tier 2 — Mechanism

## 5. The popup lifecycle

Popups are driven by two distinct calls:

- **`OpenPopup(str_id)`** — queues the popup for opening. Sets internal state so the matching `BeginPopup`/`BeginPopupModal` will succeed on its next call.
