...
## The two ConfigFlags — independent, not paired
...
## DockBuilder — programmatic layout, with a stability caveat
...
## Programmatic queries: SetNextWindowDockID, GetWindowDockID, IsWindowDocked
...
## ImGuiWindowClass — class-scoped docking
...
## .ini persistence — automatic, with manual hooks
...
**2. Layout reset on first run.** With no `imgui.ini` present, every window appears at default position and nothing is docked. Either ship a default `imgui.ini` alongside the binary, or run the DockBuilder "first-frame init" pattern shown above so the first launch is already laid out.
...
**4. Multi-viewport: surprise context loss after dragging out a window.** Symptom: the next `glfwSwapBuffers` shows a black frame or the GL state is stale. Cause: forgot to save/restore the GL context around `UpdatePlatformWindows`/`RenderPlatformWindowsDefault`. Fix shown above.
