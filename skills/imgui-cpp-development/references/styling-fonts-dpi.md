...
This mutates the persistent style struct, which is fine *outside* a frame. Inside a frame use `PushStyleColor`.

---

## 3. `ImGuiStyle` fields most users tune
...
## 4. The live theme editor: `ShowStyleEditor()`
...
4. **New default-font entry points.** `AddFontDefaultVector()` (ProggyForever, scalable) and `AddFontDefaultBitmap()` (ProggyClean, 13px pixel-perfect). `AddFontDefault()` auto-selects between them based on expected size (`imgui.h:3809-3811`, `FONTS.md:14-16`, `CHANGELOG.txt:229-235`).
...
**If your guidance came from a pre-v1.92 tutorial, expect API differences in roughly that order: glyph ranges, font sizing, `PushFont` arity, and texture management.**

---

## 7. Loading custom fonts
...
## 10. `ImFontConfig` fields that matter
...
| `GlyphExcludeRanges`     | nullptr | Exclude codepoint ranges from this font source (for resolving merge collisions, see §8).     |
...
## 11. DPI scaling — three knobs, one frequent confusion
...
This does **not** scale fonts. Pair it with `FontScaleDpi`:

### Recommended baseline (v1.92 docking)
...
ImGuiStyle& style = ImGui::GetStyle();
style.ScaleAllSizes(main_scale);        // bake once at startup
style.FontScaleDpi = main_scale;        // initial; ConfigDpiScaleFonts overrides on change
```

This matches the upstream examples' guidance (`CHANGELOG.txt:1554-1557`):
...
A known caveat from the comment at `imgui.h:2532`: `ConfigDpiScaleFonts` rescales fonts but does **not** auto-rescale spacing/padding. Re-bake those yourself if your app supports live monitor switching.

---

## 12. Multi-viewport DPI
